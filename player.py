...
class mp3Player:
    ...
    def stop(self):
        mixer.music.stop()
    def getlength(self):
        mp3 = MP3(self.folder+"//"+self.currentSong)
        return mp3.info.length
    def getPosition(self):
        return mixer.music.get_pos()/1000 #need to divide by 1000 to go from milliseonds to seconds
    def setPosition(self,pos):
        return mixer.music.set_pos(pos*1000)# multiplie by 100 to go from seconds to milliseconds
    def volPlus(self):
        self.vol += 0.01
        mixer.music.set_volume(self.vol)
    def volMinus(self):
